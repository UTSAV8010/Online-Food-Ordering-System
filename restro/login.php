...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '') {
        ...
        $sql = 'SELECT * FROM tbl_restro WHERE username = ?';
        ...
        if ($result->num_rows > 0) {
            ...
            } elseif (($user['status'] ?? '') === 'not_approved') {
                $error_message = 'Your restro has not been approved.';
            } elseif (($user['status'] ?? '') !== 'approved') {
                $error_message = 'Your account is not verified by the admin.';
            } elseif (password_verify($password, $user['password'])) {
                $_SESSION['restro-name'] = $user['restro_name'];
...
<head>
    ...
    <title>Restaurant Login | Pasar-kita</title>
...
<body>
    <div class="auth-shell">
        ...
        <p class="auth-subtitle">Sign in to manage menu, orders, and settings.</p>
