...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['verify_email'])) {
        ...
        if ($email === '') {
            ...
            $stmt = $conn->prepare('SELECT * FROM tbl_restro WHERE email = ?');
            ...
            if ($result->num_rows === 0) {
                ...
                $stmt = $conn->prepare('UPDATE tbl_restro SET reset_key = ? WHERE email = ?');
                ...
                if ($stmt->execute()) {
                    ...
                    $mailResult = app_send_password_reset_email($email, $resetKey, 'Restaurant');
    ...
    if (isset($_POST['update_password'])) {
        ...
        if ($newPassword === '' || $confirmPassword === '') {
            ...
            $stmt = $conn->prepare('UPDATE tbl_restro SET password = ?, reset_key = ? WHERE email = ?');
            $emptyResetKey = '';
            $stmt->bind_param('sss', $hashedPassword, $emptyResetKey, $email);
...
<head>
    ...
    <title>Restaurant Forgot Password | Pasar-kita</title>
...
<body>
    <div class="auth-shell">
        <div class="auth-head">
            ...
            <h1 class="auth-title">Reset Restaurant Password</h1>
