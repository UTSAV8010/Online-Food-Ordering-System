...
$revenue = "SELECT COALESCE(SUM(salary), 0) AS total_amount
...
$tip_query = "SELECT SUM(tip) AS total_tips FROM tbl_review WHERE name = '$delivery_boy_name'";
$res_tip = mysqli_query($conn, $tip_query);
$total_tip = mysqli_fetch_array($res_tip);
...
$sql_delivered_by_me = "SELECT COUNT(*) AS total FROM order_manager
                        WHERE delivery_boy_name='$delivery_boy_name'";
$res_delivered_by_me = mysqli_query($conn, $sql_delivered_by_me);
$delivered_row = ($res_delivered_by_me && ($tmp = mysqli_fetch_assoc($res_delivered_by_me))) ? (int)$tmp['total'] : 0;
...
<head>
    ...
    <script type="text/javascript">
      ...
      function setChipText() {
        ...
        const monthlyChipEl = document.getElementById('monthly-chip');
        if (!chipEl) return;
        if (!liveTimestamp) {
          chipEl.textContent = SALES_RANGE_LABEL;
          if (monthlyChipEl) monthlyChipEl.textContent = MONTHLY_RANGE_LABEL;
          return;
        }
        const time = new Date(liveTimestamp);
        if (monthlyChipEl) {
          monthlyChipEl.textContent = `Updated ${time.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })} - ${MONTHLY_RANGE_LABEL}`;
        }
...
<section id="content">
    ...
    <main>
        <div class="dashboard-grid">
          <a href="manage-review.php" class="kpi-card kpi-card-blue">
            <div class="kpi-media"><img src="../images/inventory.png" alt="Tips"><span class="kpi-badge">Live</span></div>
            <div class="kpi-meta"><h3 class="kpi-value" id="kpi-categories"><?php echo number_format((float)($total_tip['total_tips'] ?? 0), 2); ?></h3><p class="kpi-label">Total Tips</p></div>
          </a>
          ...
          <a href="monthly-revenue.php" class="kpi-card kpi-card-gold">
            ...
            <div class="kpi-meta"><h3 class="kpi-value" id="kpi-revenue">Rs <?php echo number_format((float)($total_revenue['total_amount'] ?? 0), 2); ?></h3><p class="kpi-label">Monthly Revenue</p></div>
          </a>

          <a href="manage-online-order.php?remaining=1" class="kpi-card kpi-card-violet">
            <div class="kpi-media"><img src="../images/orders_completed.png" alt="Remaining Orders"><span class="kpi-badge">Live</span></div>
            <div class="kpi-meta"><h3 class="kpi-value" id="kpi-orders"><?php echo number_format((int)$total_orders_delivered); ?></h3><p class="kpi-label">Delivery Remaining Orders</p></div>
          </a>

          <a href="manage-online-order.php?status=Delivered&mine=1" class="kpi-card kpi-card-red">
            <div class="kpi-media"><img src="../images/folder2.png" alt="Delivered"><span class="kpi-badge">Live</span></div>
            <div class="kpi-meta"><h3 class="kpi-value" id="kpi-menu"><?php echo number_format((int)$delivered_row); ?></h3><p class="kpi-label">Order Delivered By You</p></div>
          </a>
        </div>

        <div class="dashboard-charts">
          <div class="chart-panel">
            <div class="panel-head"><h3>Deliveries By Status</h3><span class="panel-chip">Live Data</span></div>
            <div class="chart" id="donutchart_msi"></div>
          </div>
          <div class="chart-panel">
            <div class="panel-head"><h3>Earnings By Day</h3><span class="panel-chip" id="sales-chip">Last 7 days</span></div>
