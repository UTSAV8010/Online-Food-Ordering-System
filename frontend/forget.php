...
if (!function_exists('frontend_clear_password_reset_state')) {
    function frontend_clear_password_reset_state(): void
    {
        unset(
            $_SESSION['verified_email'],
            $_SESSION['reset_key'],
            $_SESSION['reset_key_expires_at'],
            $_SESSION['reset_verified']
        );
    }
}

if (!function_exists('frontend_issue_reset_key')) {
    function frontend_issue_reset_key(mysqli $conn, string $email): array
    {
        $resetKey = (string) random_int(100000, 999999);
        $stmt = $conn->prepare("UPDATE tbl_users SET reset_key = ? WHERE email = ?");

        if (!$stmt) {
            return array('success' => false, 'error' => 'Unable to start password reset. Please try again.');
        }

        $stmt->bind_param("ss", $resetKey, $email);
        if (!$stmt->execute()) {
            return array('success' => false, 'error' => 'Unable to generate reset key. Please try again.');
        }

        $mailResult = app_send_password_reset_email($email, $resetKey, 'Customer');
        if (!$mailResult['success']) {
            $clearStmt = $conn->prepare("UPDATE tbl_users SET reset_key = NULL WHERE email = ?");
            if ($clearStmt) {
                $clearStmt->bind_param("s", $email);
                $clearStmt->execute();
            }
            frontend_clear_password_reset_state();
            return array('success' => false, 'error' => $mailResult['error']);
        }

        $_SESSION['verified_email'] = $email;
        $_SESSION['reset_key'] = $resetKey;
        $_SESSION['reset_key_expires_at'] = time() + APP_OTP_TTL_SECONDS;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['verify_email'])) {
        ...
        if ($email === '') {
            ...
            $stmt = $conn->prepare("SELECT id FROM tbl_users WHERE email = ? LIMIT 1");
            ...
            if ($result->num_rows === 0) {
                ...
                $issueResult = frontend_issue_reset_key($conn, $email);

                if ($issueResult['success']) {
                    $message = "<div class='alert-box success-box'>Reset key sent to your registered email. It expires in " . APP_OTP_TTL_SECONDS . " seconds.</div>";
                } else {
                    $message = "<div class='alert-box error-box'>" . htmlspecialchars($issueResult['error'], ENT_QUOTES, 'UTF-8') . "</div>";
                }
            }
        }
    } elseif (isset($_POST['resend_reset_key'])) {
        $email = trim((string) ($_SESSION['verified_email'] ?? ''));
        ...
        if ($email === '') {
            ...
            if ($issueResult['success']) {
                $message = "<div class='alert-box success-box'>A new reset key has been sent. It expires in " . APP_OTP_TTL_SECONDS . " seconds.</div>";
            } else {
                $message = "<div class='alert-box error-box'>" . htmlspecialchars($issueResult['error'], ENT_QUOTES, 'UTF-8') . "</div>";
            }
        }
    } elseif (isset($_POST['verify_reset_key'])) {
        ...
        } elseif ($resetKey !== (string) $_SESSION['reset_key']) {
            $message = "<div class='alert-box error-box'>Reset key does not match.</div>";
        } else {
            $_SESSION['reset_verified'] = true;
            unset($_SESSION['reset_key'], $_SESSION['reset_key_expires_at']);
        ...
        } elseif (!isset($_SESSION['verified_email'], $_SESSION['reset_verified']) || $_SESSION['reset_verified'] !== true) {
            $message = "<div class='alert-box error-box'>Please verify reset key first.</div>";
        } else {
            $hashedPassword = password_hash($newPassword, PASSWORD_BCRYPT);
            $email = $_SESSION['verified_email'];

            $stmt = $conn->prepare("UPDATE tbl_users SET password = ?, reset_key = NULL WHERE email = ?");
...
$showPasswordField = isset($_SESSION['verified_email'], $_SESSION['reset_verified']) && $_SESSION['reset_verified'] === true;
$showResetKeyField = !$showPasswordField && isset($_SESSION['verified_email'], $_SESSION['reset_key'], $_SESSION['reset_key_expires_at']);
$resetExpiresAt = (int) ($_SESSION['reset_key_expires_at'] ?? 0);

if ($showResetKeyField && $resetExpiresAt > 0 && time() > $resetExpiresAt && $message === '') {
    $message = "<div class='alert-box error-box'>Reset key expired. Click resend to get a new code.</div>";
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Forgot Password | Pasar-kita</title>
    ...
    <div class="auth-shell">
        ...
        <?php if ($showResetKeyField && !$showPasswordField): ?>
            ...
            <form method="post" id="keyStepForm" novalidate>
                <div class="field-group">
                    <label class="field-label" for="reset_key_digit_1">6-digit Reset Key</label>
                    <div class="otp-panel">
                        <input class="otp-hidden" type="text" name="reset_key" id="reset_key" maxlength="6" value="<?php echo isset($_POST['reset_key']) ? htmlspecialchars($_POST['reset_key']) : ''; ?>" inputmode="numeric" autocomplete="one-time-code" tabindex="-1" required>
                        <div class="otp-grid" data-otp-group data-target="reset_key">
                            <input class="otp-digit" type="text" id="reset_key_digit_1" inputmode="numeric" maxlength="1" placeholder="0" autocomplete="one-time-code">
                    ...
                    <div class="otp-status-row" data-otp-status data-expires-at="<?php echo (int) $resetExpiresAt; ?>">
                        ...
                        <div class="otp-resend-wrap">
                            ...
                            <button class="otp-link-btn" type="submit" name="resend_reset_key" formnovalidate>Resend</button>
                        </div>
                    </div>
                    <div class="otp-expired-note" id="resetOtpExpiredNote">Reset key expired. Click resend to get a new code.</div>
                </div>
                <button class="auth-btn" type="submit" name="verify_reset_key" data-otp-verify>Verify Reset Key</button>
            </form>
        <?php endif; ?>
    ...
    <script>
        (function () {
            ...
            if (keyForm) {
                ...
                const otpGroup = document.querySelector('[data-otp-group][data-target="reset_key"]');
                const otpStatus = keyForm.querySelector('[data-otp-status]');
                const otpTimer = keyForm.querySelector('[data-otp-timer]');
                const expiredNote = document.getElementById('resetOtpExpiredNote');
                const verifyButton = keyForm.querySelector('[data-otp-verify]');
                ...
                if (otpGroup) {
                    ...
                    const syncKeyValue = () => {
                        key.value = digits.map((input) => input.value).join('');
                    };

                    syncDigitsFromValue(key.value);
                ...
                keyForm.addEventListener("submit", (e) => {
                    ...
                    if (submitter && submitter.name === 'resend_reset_key') {
                        return;
                    }

                    if (timerExpired) {
                        keyError.textContent = 'Reset key expired. Click resend to get a new code.';
                        e.preventDefault();
                        return;
                    }
