...
<body>
    ...
    <div class="head-title">
        <div class="left">
            <h1>Update Festival Coupons</h1>
            <ul class="breadcrumb">
                <li><a class="clickable" href="index.php">Dashboard</a></li>
                <li><i class='bx bx-chevron-right'></i></li>
                <li><a class="clickable" href="manage-fest-coupon.php"> Festival Coupons</a></li>
                <li><i class='bx bx-chevron-right'></i></li>
                <li><a class="active" href="update-fest-coupon.php">Update Festival Coupons</a></li>
            </ul>
        </div>
    </div>
    <br/>
    <?php 
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $sql = "SELECT * FROM tbl_fest_coupon WHERE id=?";
            ...
            <div class="head">
                <form id="updateForm" action="" method="POST">
                    <table>
                        <tr>
                            <td>Name:</td>
                            <td><input id="ip2" type="text" name="name" value="<?php echo $name; ?>">
                                <span class="error"></span></td>
                        </tr>
                        <tr>
                            <td>Coupon Code:</td>
                            <td><input type="text" id="ip2" name="coupon_code" value="<?php echo $coupon_code; ?>">
                                <span class="error"></span></td>
                        </tr>
                        <tr>
                            <td>Discount (%):</td>
                            <td><input type="number" id="ip2" name="discount" value="<?php echo $discount; ?>">
                                <span class="error"></span></td>
                        </tr>
                        <tr>
                            <td>Duration (days):</td>
                            <td><input type="number" id="ip2" name="duration" value="<?php echo $duration; ?>">
                                <span class="error"></span></td>
                        </tr>
                        <tr>
                            <td>Active:</td>
                        ...
                        <tr>
                            <td>
                                ...
                                <input type="submit" name="submit" value="Update Coupon" class="button-8" role="button">
                            </td>
                        </tr>
                    </table>
                </form>
                <?php 
                    if (isset($_POST['submit'])) {
                        ...
                        $sql2 = "UPDATE tbl_fest_coupon SET festival_name=?, coupon_code=?, discount=?, duration=?, status=? WHERE id=?";
                        $stmt2 = mysqli_prepare($conn, $sql2);
                        mysqli_stmt_bind_param($stmt2, "ssddsi", $name, $coupon_code, $discount, $duration, $active, $id);
...
<script>
    document.getElementById('updateForm').addEventListener('submit', function(event) {
        ...
        let coupon_code = document.querySelector('[name="coupon_code"]');
        if (coupon_code.value.trim() === '') {
            coupon_code.nextElementSibling.innerText = 'Coupon code is required';
            valid = false;
        }
        
        let discount = document.querySelector('[name="discount"]');
        if (discount.value === '' || discount.value < 0 || discount.value > 100) {
            discount.nextElementSibling.innerText = 'Enter a valid discount (0-100%)';
            valid = false;
        }
        
        let duration = document.querySelector('[name="duration"]');
        if (duration.value === '' || duration.value <= 0) {
            duration.nextElementSibling.innerText = 'Enter a valid duration';
            valid = false;
        }
        
        if (!document.querySelector('[name="active"]:checked')) {
            document.querySelector('[name="active"]').nextElementSibling.innerText = 'Select status';
            valid = false;
        }
