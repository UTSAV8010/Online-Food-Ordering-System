...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['verify_email'])) {
        ...
        if ($email === '') {
            ...
            $stmt = $conn->prepare('SELECT id FROM tbl_admin WHERE email = ? LIMIT 1');
            ...
            if ($result->num_rows === 0) {
                ...
                $_SESSION['reset_key_expires_at'] = time() + 600; // 10 minutes

                $mailResult = app_send_password_reset_email($email, $resetKey, 'Admin');
...
<head>
    ...
    <title>Admin Forgot Password | Pasar-kita</title>
