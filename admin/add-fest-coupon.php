...
<body>
    ...
    <div class="head-title">
        <div class="left">
            ...
            <ul class="breadcrumb">
                ...
                <li>
                    <a class="clickable" href="manage-fest-coupon.php">Festival Coupons</a>
                </li>
                <li><i class='bx bx-chevron-right'></i></li>
                <li>
                    <a class="active" href="add-fest-coupon.php">Add Festival Coupon</a>
                </li>
            </ul>
        </div>
    </div>
    ...
    <script>
        function validateForm() {
            ...
            let festivalName = document.forms["couponForm"]["festival_name"].value.trim();
            if (festivalName === "") {
                document.getElementById("festivalNameError").innerHTML = "Festival Name is required";
                isValid = false;
            }

            let couponCode = document.forms["couponForm"]["coupon_code"].value.trim();
            if (couponCode === "") {
                document.getElementById("couponCodeError").innerHTML = "Coupon Code is required";
                isValid = false;
            }

            let discount = document.forms["couponForm"]["discount"].value.trim();
            if (discount === "" || isNaN(discount) || discount < 0 || discount > 100) {
                document.getElementById("discountError").innerHTML = "Enter a valid discount (0-100%)";
                isValid = false;
            }

            let duration = document.forms["couponForm"]["duration"].value.trim();
            if (duration === "" || isNaN(duration) || duration <= 0) {
                document.getElementById("durationError").innerHTML = "Enter a valid duration (minimum 1 day)";
                isValid = false;
            }
    ...
    <div class="table-data">
        <div class="order">
            <div class="head">
                <form name="couponForm" action="" method="POST" onsubmit="return validateForm()">
                    <table class="rtable">
                        <tr>
                            <td>Festival Name</td>
                            <td>
                                <input type="text" name="festival_name" id="ip2">
                                <span class="error-message" id="festivalNameError"></span>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td>Coupon Code</td>
                            <td>
                                <input type="text" id="ip2" name="coupon_code">
                        ...
                        <tr>
                            <td>Discount (%)</td>
                            <td>
                                <input type="number" id="ip2" name="discount" step="0.01" min="0" max="100">
                        ...
                        <tr>
                            <td>Duration (Days)</td>
                            <td>
                                <input type="number" id="ip2" name="duration" min="1">
                        ...
                        <tr>
                            <td colspan="2">
                                <input type="submit" name="submit" value="Add Coupon" class="button-8" role="button">
                            </td>
                        </tr>
                    </table>
                </form>
    ...
    if (isset($_POST['submit'])) {
        ...
        // Insert into tbl_fest_coupon
        $sql = "INSERT INTO tbl_fest_coupon (festival_name, coupon_code, discount, duration, status, expire) VALUES (?, ?, ?, ?, ?, 'active')";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ssdds", $festival_name, $coupon_code, $discount, $duration, $status);
        $res = mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);

        if ($res) {
            $_SESSION['add'] = "<div class='success text-center'>Festival Coupon Added Successfully</div>";
            header('location:' . SITEURL . 'manage-fest-coupon.php');
            exit();
        } else {
            $_SESSION['add'] = "<div class='error text-center'>Failed to Add Festival Coupon</div>";
            header('location:' . SITEURL . 'add-fest-coupon.php');
            exit();
        }
    }
    ?>
